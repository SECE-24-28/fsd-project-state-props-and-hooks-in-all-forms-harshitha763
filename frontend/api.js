...
async function loadWishlistIds() {
  ...
  document.querySelectorAll('.wish-btn').forEach(btn => {
    ...
    btn.classList.toggle('active', _wishlistIds.includes(id));
    const icon = btn.querySelector('i');
    if (icon) icon.className = _wishlistIds.includes(id) ? 'fas fa-heart text-danger' : 'far fa-heart';
  });
}
...
async function toggleWishlist(productId) {
  try {
    ...
    document.querySelectorAll(`.wish-btn[data-id="${productId}"]`).forEach(btn => {
      btn.classList.toggle('active', _wishlistIds.includes(productId));
      const icon = btn.querySelector('i');
      if (icon) icon.className = _wishlistIds.includes(productId) ? 'fas fa-heart text-danger' : 'far fa-heart';
    });
  } catch (e) { showToast(e.message, 'danger'); }
}

function isWishlisted(id) { return _wishlistIds.includes(id); }
  ...
  <div class="col-6 col-md-4 col-lg-3">
    <div class="card product-card h-100">
      <div class="position-relative" style="overflow:hidden">
        <img src="${p.image}" class="card-img-top" alt="${p.name}" style="cursor:pointer" onclick="openProductModal(${p.id})">
        ...
        <button class="wish-btn ${wishlisted ? 'active' : ''}" data-id="${p.id}" onclick="toggleWishlist(${p.id})">
      ...
      <div class="card-body text-center px-2 pb-3">
        ...
        <h6 class="prod-name" style="cursor:pointer" onclick="openProductModal(${p.id})">${p.name}</h6>
        ...
        <button class="btn-add-cart" onclick="addToCart(${p.id})"><i class="fas fa-cart-plus me-1"></i>Add to Cart</button>
      </div>
    </div>
  </div>`;
}
